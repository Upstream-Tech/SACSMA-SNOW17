...
subroutine spin_up_first_year(a, params_in,spinup_crit, in_tair,in_precip,in_pet, in_raim, uztwc, uzfwc, lztwc, &
                              lzfsc, lzfpc, adimc)
  use nrtype
...
!input variables
  ...
  real(dp),intent(in),dimension(36500)	:: in_tair,in_pet,in_precip,in_raim
  real(sp),intent(in),dimension(30)	:: params_in
...
!print *,in_tair(12),in_precip(12),in_pet(12),in_raim(10:15)
...
!  print *,'before',uztwc_sp,uzfwc_sp,lztwc_sp,lzfsc_sp,lzfpc_sp,adimc_sp
    ...
    !set single precision inputs
      tair_sp   = real(in_tair(i),kind(sp))
      precip_sp = real(in_precip(i),kind(sp))
      pet_sp    = real(in_pet(i),kind(sp))
      fuse_sp   = real(in_raim(i),kind(sp))
            !precip multiplier
